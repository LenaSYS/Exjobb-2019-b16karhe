...
<html>
    ...
    <body>
        ...
        <div id="cartApp"> <!-- Link to the ViewModel which starts Vue. -->
            ...
            <ol>
                <h3>PRIS: {{ priceCart }} kr <label v-if="newProduct">+ {{newProduct.price}}</label> </h3>
                <div id="theCart">
                <li class="lista" v-for="(product, index) in inCart"> <!-- Listing of items inside products as product. The data is retrived from the model through the ViewModel. -->
                    {{ product.stockCode }} -
                    "{{ product.description }}" -
                    {{ product.price }} kr/st
            ...
            <form id="outerFlex" v-on:submit.prevent="newProduct = false">
                <h2>Liknande produkter:</h2>
                <div class="innerFlex" v-for="(product, index) in outCart"  v-if="index < 5"> <!-- Listing 5 prodcuts outside of the cart by limiting the itteration through the index -->
                ...
                <div id="addButtonDiv">
                    <button id="addButton" :disabled="newProduct == false" v-on:click="outCart[id].selectedQuantity = 1">
                        Add product
                    </button>
                </div>
            </form>
